...
if [[ -z ${BASE_REF:-} ]]; then
	...
	nix eval -vv --json --expr "let f = builtins.getFlake \"path:.\"; in [ ${drv_items} ]" >"$tmpdir/current.json" &
	nix eval -vv --json --expr "let f = builtins.getFlake \"${BASE_REF}\"; in [ ${drv_items} ]" >"$tmpdir/base.json" &
	wait
