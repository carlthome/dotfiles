...
fzf --ansi --disabled --query "$INITIAL_QUERY" \
	--prompt 'rg blame> ' \
	--header '^T: rg/fzf | ^G: show commit | ?: preview' \
	...
	--preview 'git blame -L {2},{2} {1} 2>/dev/null; echo; BAT_THEME=ansi bat --color=always {1} --highlight-line {2} --style=plain 2>/dev/null' \
